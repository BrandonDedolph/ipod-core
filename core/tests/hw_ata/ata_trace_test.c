...
static void test_power(xfail_ctx *c)
{
    ...
    expect_timed_wait(&tc, 0);                    /* completed (BSY clear) */
    finish(c, &tc);
    ...
    expect_w(&tc, 8, ATA_CONTROL_ADDR, ATA_CONTROL_NIEN);
    expect_w(&tc, 8, ATA_SELECT_ADDR, ATA_SELECT_OBS);
    expect_timed_wait(&tc, 0);                    /* post-SRST spin-up wait */
    expect_wait_ready(&tc);
    expect_command(&tc, 0, ATA_PHYS_LOG, ATA_CMD_READ_SECTORS);
    expect_sector_in(&tc, 0);
    expect_sector_in(&tc, 0);
    ...
    {
        /* Timed grammar: ready poll, BSY-clear, then the ERR/DF read is the
         * THIRD status read; the last value persists after the queue ends. */
        static const uint32_t seq[] = { RDY, RDY, RDY | ERR, RDY };
        mmio_mock_reset();
        mmio_mock_queue_read(ATA_ALT_STATUS_ADDR, seq, 4);
        mmio_mock_set_read(ATA_ERROR_ADDR, ERROR_ABRT);
        rc = ata_sleep();
     ...
     * Same shape as the STANDBY case: the flush's ready wait gives up on the
     * 10 s ceiling (timer 0, ==10 s, >10 s) and nothing is issued. */
    {
        static const uint32_t usec[] = { 0, 0, READY_US, READY_US + 1u };
        mmio_mock_reset();
        mmio_mock_set_read(ATA_ALT_STATUS_ADDR, BSY);
        ...
        xpect(c, "sleep: no command was issued to it",
              count_writes(ATA_COMMAND_ADDR) == 0);
        tc = trace_begin("sleep-busy-forever");
        expect_timed_timeout(&tc, 3);   /* 0, ==10 s, >10 s */
        finish(c, &tc);
    }
