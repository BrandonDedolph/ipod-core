...
/*
 * tests/codecs/flac_freestanding_test.c — host test for the streaming FLAC
 * decode path used on hw.
 *
 * Exercises exactly what the device will do, minus the disk: open the KAT
 * FLAC through flac_open_stream() (a decoder_source_t pull source, here backed
 ...
 * (the same sine_440hz_1s reference codec_kat uses). Also reports the arena
 * high-water mark so PR2 can size the on-device arena from a real decode.
 *
 * The code under test (flac.c + arena.c) is the SAME source the ARM build
...
int main(int argc, char **argv)
{
    ...
    snprintf(p_flac, sizeof p_flac,
             "%s/sine_440hz_1s_44k_s16_stereo.flac", argv[1]);
    snprintf(p_pcm, sizeof p_pcm,
             "%s/sine_440hz_1s_44k_s16_stereo.pcm", argv[1]);

    size_t flac_len = 0, ref_len = 0;
    uint8_t *flac_bytes = slurp(p_flac, &flac_len);
    uint8_t *ref_pcm    = slurp(p_pcm, &ref_len);
    if (!flac_bytes || !ref_pcm) {
    ...
    membuf_t mb = { flac_bytes, flac_len, 0 };
    decoder_source_t src = { mem_read, mem_seek, mem_tell, &mb };

    decoder_t d;
    int rc = flac_open_stream(&d, &src, &alloc);
    fails += check("flac_open_stream returns DECODER_OK", rc == DECODER_OK);
