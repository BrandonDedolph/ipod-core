...
type Plan struct {
	...
	Log    bool `json:"log"`
	// Clock says the sync will stamp the host's time into CORECFG.DAT for
	// the device to pick up at its next boot. Always true for a real run:
	// the iPod cannot be told the time any other way (internal/devicefs/
